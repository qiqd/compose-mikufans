...
@Serializable
class CommonTemplate : HtmlParser {
  private val blankReg = "[\\s\u3000]+"

  override val name: String = "通用模板"
  override val logoUrl: String = ""
  override val baseUrl: String = ""
  fun getBlankReg(): String = blankReg
  fun setBlankReg(blankReg: String) {
    this.blankReg = blankReg
  }

  override fun getSearchResult(searchUrl: String, page: Int?, size: Int?): List<Anime> {
    ...
    try (
      val response = client.newCall(request).execute()) {
        ...
        for (element in elements) {
          ...
          val type = element.select("div.thumb-else a").map(Element::text).joinToString("•")
          val director = element.select("div.thumb-director a:not(:first-child)").map(Element::text)
            .joinToString("•")
          val actor = element.select("div.thumb-actor a:not(:first-child)").map(Element::text)
            .joinToString("•")
          val description =
